"""Parser for Home Assistant automations.yaml."""
...
PARSER_NAME = "automations"
SOURCE_FILE = "automations.yaml"

# Keys whose string values are service/action identifiers, not entity IDs.
SERVICE_KEYS = {"action", "service"}


class AutomationsFormatError(RuntimeError):
    """Raised when automations.yaml has an unsupported structure."""


def parse_automation_relations(
    automations_path: str | Path,
    entity_nodes: list[Node],
) -> ParserResult:
    """Parse automation definitions into dependency edges."""
    ...
    if not isinstance(payload, list):
        raise AutomationsFormatError(
            f"Expected top-level list in {path}, got {type(payload).__name__}"
        )

    automation_by_unique_id = _automation_index(entity_nodes)
    known_entity_ids = {
    ...
    for index, definition in enumerate(payload):
        source_path = f"[{index}]"

        if not isinstance(definition, dict):
            result.warnings.append(
                f"{source_path}: skipped non-object automation"
            )
            continue

        automation_id = definition.get("id")
        if automation_id is None:
            result.warnings.append(
                f"{source_path}: automation missing id"
            )
            continue

        automation_entity_id = automation_by_unique_id.get(str(automation_id))

        if automation_entity_id is None:
            result.warnings.append(
                f"{source_path}: no automation entity found for id "
                f"{automation_id}"
            )
            continue
        ...
        conditions = definition.get(
            "conditions", definition.get("condition", [])
        )
        actions = definition.get("actions", definition.get("action", []))

        _parse_incoming_section(
            result=result,
            value=triggers,
            automation_entity_id=automation_entity_id,
            known_entity_ids=known_entity_ids,
            relation_type=RelationType.TRIGGERS,
            source_path=f"{source_path}.triggers",
            context="trigger",
        )
        ...
        _parse_incoming_section(
            ...
            source_path=f"{source_path}.conditions",
            context="condition",
        )
        ...
        _parse_actions(
            ...
            automation_entity_id=automation_entity_id,
            known_entity_ids=known_entity_ids,
            source_path=f"{source_path}.actions",
        )

    return result


def _automation_index(entity_nodes: Iterable[Node]) -> dict[str, str]:
    index: dict[str, str] = {}

    for node in entity_nodes:
        if node.node_type != "entity":
            continue
        if node.metadata.get("platform") != "automation":
            continue

        unique_id = node.metadata.get("unique_id")
        if isinstance(unique_id, str) and unique_id:
            index[unique_id] = node.node_id

    return index


def _parse_incoming_section(
    *,
    result: ParserResult,
    value: Any,
    ...
    references = _collect_references(value, source_path)

    for entity_id, confidence, ref_path, explanation in references:
        _append_edge(
            result=result,
            source_node_id=entity_id,
            target_node_id=automation_entity_id,
    ...
    *,
    ...
    for item, item_path in _walk(value, source_path):
        ...
        for entity_id, ref_path in targets:
            ...
            _append_edge(
                result=result,
                source_node_id=automation_entity_id,
                target_node_id=entity_id,
                relation_type=relation_type,
                confidence=Confidence.CERTAIN,
                known_entity_ids=known_entity_ids,
                source_path=ref_path,
                explanation="action target entity",
            )
        ...
        for entity_id, confidence, ref_path, explanation in references:
            ...
            _append_edge(
                ...
                known_entity_ids=known_entity_ids,
                source_path=ref_path,
                explanation=f"action input: {explanation}",
            )
...
def _append_edge(
    *,
    ...
    if unknown_id is not None:
        result.warnings.append(
            f"{source_path}: automation relation references unknown entity "
            f"{unknown_id}"
        )
