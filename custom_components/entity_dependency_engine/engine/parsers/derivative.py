"""Parser for GUI-created derivative entities."""
    ...
    for index, entry in enumerate(entries):
        ...
        if not isinstance(source_entity, str) or not source_entity.strip():
            result.warnings.append(
                f"{source_path}: derivative has no valid source entity"
            )
            continue
        ...
        if not targets:
            result.warnings.append(
                f"{source_path}: no entity found for derivative {entry_id}"
            )
            continue

        if source_entity not in known_entity_ids:
            result.warnings.append(
                f"{source_path}: derivative references unknown entity "
                f"{source_entity}"
            )
        ...
        for target in targets:
            result.edges.append(
                Edge(
                    ...
                    explanation=(
                        "derivative options.source feeds the derivative entity"
                    ),
                    metadata={
