"""Parser for GUI-created history_stats entities."""
    ...
    for index, entry in enumerate(entries):
        ...
        if not isinstance(source_entity, str) or not source_entity.strip():
            result.warnings.append(
                f"{source_path}: history_stats has no valid entity_id"
            )
            continue
        ...
        if not targets:
            result.warnings.append(
                f"{source_path}: no entity found for history_stats {entry_id}"
            )
            continue

        if source_entity not in known_entity_ids:
            result.warnings.append(
                f"{source_path}: history_stats references unknown entity "
                f"{source_entity}"
            )
        ...
        for target in targets:
            result.edges.append(
                Edge(
                    ...
                    explanation=(
                        "history_stats options.entity_id is the tracked entity"
                    ),
                    metadata={
