"""Parser for GUI-created integration (Riemann sum) entities."""
    ...
    for index, entry in enumerate(entries):
        ...
        if not isinstance(source_entity, str) or not source_entity.strip():
            result.warnings.append(
                f"{source_path}: integration sensor has no valid source"
            )
            continue
        ...
        if not targets:
            result.warnings.append(
                f"{source_path}: no entity found for integration {entry_id}"
            )
            continue

        if source_entity not in known_entity_ids:
            result.warnings.append(
                f"{source_path}: integration references unknown entity "
                f"{source_entity}"
            )
        ...
        for target in targets:
            result.edges.append(
                Edge(
                    ...
                    explanation=(
                        "integration options.source feeds the accumulated entity"
                    ),
                    metadata={
                        "method": options.get("method"),
                        "unit_prefix": options.get("unit_prefix"),
                        "unit_time": options.get("unit_time"),
                        "round": options.get("round"),
                        "max_sub_interval": options.get("max_sub_interval"),
                    },
                )
            )
